...
1. Open Clark and go to the **Win ISO Creator** tab.
...
2. Clark mounts the ISO, checks for a valid `install.wim` or `install.esd`, and reads the available editions (Home, Pro, Enterprise, etc.). The image must contain at least one **Windows 10** or **Windows 11** client edition (Windows Server ISOs are not supported).
...
Click **Run Windows ISO Modification and Creator** to start the customization process. Clark will:
...
A live log shows progress as each step completes. This stage usually takes **10–30 minutes** depending on disk speed. The WIM dismount near the end is the slowest part, so do not close Clark while it is running.
...
{{< tabs >}}
  ...
  3. Clark builds a dual BIOS/UEFI bootable ISO using `oscdimg.exe`.



  > [!NOTE]

  > `oscdimg.exe` (part of the Windows ADK) is required. If it's not found, Clark will attempt to install it automatically via winget. If that fails, install it manually: `winget install -e --id Microsoft.OSCDIMG`
  ...
  **Typical output size:** 2.5–3.5 GB (down from 5–6 GB original) {{< /tab >}}
  ...
  4. Clark formats the drive as GPT with a 512 MB EFI partition and copies the modified Windows files.
...
| "Access Denied" error | Make sure Clark is running as Administrator |
...
| **[MicroWin](https://github.com/CodingWonders/microwin)** | A C# desktop app for building stripped-down, customized Windows ISOs — the original predecessor to Clark's old MicroWin feature | [github.com](https://github.com/CodingWonders/microwin) |
...
> Already have a Windows 10 or 11 ISO? Skip the third-party tools and use Clark's built-in **[Win ISO Creator](#using-clarks-win11-creator)** at the top of this page.
...
> Newer Windows 11 ISOs may not boot correctly on older versions of Ventoy — make sure Ventoy is up to date before use. If issues persist after updating, this is a Ventoy compatibility limitation outside of Clark's control.
