...
Clark is a PowerShell-based Windows utility with a WPF (Windows Presentation Foundation) GUI. This document explains the architecture, code structure, and how different components work together.
...
├── clark.ps1                 # Compiled output (generated)
...
│   │   ├── Get-ClarkVariables.ps1

│   │   ├── Install-ClarkWinget.ps1
...
**Purpose**: Combines all separate script files into a single `clark.ps1` for distribution.
...
- `Install-ClarkProgram*`: Installs applications



**Naming Convention**: Functions start with `WPF` or `Clark` to be loaded into the runspace.
...
- `Get-ClarkVariables.ps1`: Retrieves UI element references

- `Install-ClarkWinget.ps1`: Ensures WinGet is installed

- `Get-ClarkCheckBoxes.ps1`: Gets checkbox states

- `Invoke-ClarkCurrentSystem.ps1`: Gets system information
...
## Win ISO Creator Architecture



The **Win ISO Creator** (UI: **Win ISO Creator** tab; code and `$sync` keys may still use `Win11ISO` prefixes) is a specialized subsystem within Clark that creates customized **Windows 10** or **Windows 11** ISOs. It operates independently from the main package installation and tweak system.



### Win ISO Creator Components



**Core Functions** (`functions/private/`):

- `Invoke-ClarkISO.ps1`: Main orchestrator containing all Win ISO Creator functions

  - `Invoke-ClarkISOBrowse`: ISO file selection dialog

  - `Invoke-ClarkISOMountAndVerify`: Validates and mounts ISO, verifies image contains a Windows 10 or Windows 11 **client** edition (not Windows Server)

  - `Invoke-ClarkISOModify`: Launches modification in background runspace

  - `Invoke-ClarkISOExport`: Handles ISO and USB export

  - `Invoke-ClarkISOCheckExistingWork`: Recovers incomplete work sessions

  - `Invoke-ClarkISOCleanAndReset`: Cleans up temp directories and resets UI



- `Invoke-ClarkISOScript.ps1`: Applies modifications to mounted install.wim

  - Removes provisioned AppX packages (40+ bloatware apps; availability varies by Windows build)

  - Injects drivers (optional) from the current system

  - Removes OneDrive setup files

  - Applies offline registry tweaks (hardware bypass where applicable, privacy, telemetry, OOBE)
...
### Win ISO Creator Data Flow



```

User selects official Windows 10 or Windows 11 ISO

    ↓

Invoke-ClarkISOBrowse → OpenFileDialog, validates file size
...
Invoke-ClarkISOMountAndVerify
    ...
    ├─ Require at least one client edition named Windows 10 or Windows 11 (exclude Windows Server)
...
Invoke-ClarkISOModify (runs in background runspace)

    ├─ Create work directory under %TEMP% (e.g. *Win11ISO* timestamped folder; exact prefix may vary by fork)

    ├─ Copy ISO contents to disk (~5-6 GB)

    ├─ Mount install.wim at selected edition/index

    ├─ Invoke-ClarkISOScript:
    ...
    └─ Report completion, enable export options; store built edition name for default export filename

    ↓

Invoke-ClarkISOExport (user chooses output)

    ├─ Option 1: Save as ISO

    │   ├─ Build bootable ISO via oscdimg.exe (BIOS/UEFI dual-boot)

    │   └─ Default name: Win10_Modified_[date].iso or Win11_Modified_[date].iso (by edition); ~2.5-3.5 GB
...
Invoke-ClarkISOCleanAndReset (optional)
...
### Win ISO Creator Validation & Safety



**ISO Validation**:

- Intended for official Microsoft **Windows 10** or **Windows 11** client ISOs

- Validates presence of install.wim or install.esd

- Checks image metadata for a Windows 10 or Windows 11 client edition; rejects Windows Server

- Custom or pre-modified ISOs are unsupported (may fail unpredictably)
...
- Logged to a `*Win11ISO*.log` file under the work directory for debugging (exact name may vary by build)

- DISM handles image dismount with automatic cleanup on error



### Win ISO Creator Registry Tweaks



The `Invoke-ClarkISOScript` function applies **50+ offline registry tweaks**:
...
- Disable Chat icon from the Taskbar (where present)

- Disable OneDrive folder backup

- Disable Copilot (Windows 11–oriented)
...
Get-ClarkCheckBoxes → Retrieves selected apps
...
Install-ClarkWinget/Choco (if needed)

    ↓

Install-ClarkProgramWinget/Choco → Install app
...
Get-ClarkCheckBoxes → Get selected tweaks
...
Clark uses PowerShell runspaces for the GUI to remain responsive:
...
Clark uses PowerShell error handling:
...
At startup, Clark loads all configurations:
...
2. File naming must include "WPF" or "Clark" to load
...
Outputs `clark.ps1` in the root directory.
...
2. GitHub Actions builds and uploads `clark.ps1`
...
- Public functions: `Invoke-WPF*.ps1` or `Invoke-Clark*.ps1`

- Private functions: `Get-Clark*.ps1` or verb-Clark*.ps1`

- Must include "WPF" or "Clark" to load
...
- **GitHub Repository**: [ChrisTitusTech/clark](https://github.com/ChrisTitusTech/clark)
