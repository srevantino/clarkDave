...
- Entries with **services** get the `Set-ClarkService.ps1` function appended
...
The script strips common prefixes from the JSON key names using the pattern `WPF(Clark|Toggle|Features?|Tweaks?|Panel|Fix(es)?)?`. For example:
